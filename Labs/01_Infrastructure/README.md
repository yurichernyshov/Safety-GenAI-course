...
- Анализ характеристик и возможностей больших языковых моделей (LLM) на примере qwen	2
- Создание виртуального сервера для LLM	4
- Виртуальные машины на локальном компьютере	4
- Yandex Cloud	5
- Создать виртуальную машину	5
- Установка системного программного обеспечения	9
- Фреймворки для установки и использования LLM	10
- ollama	10
- Vllm + AWQ	11
- Установка без GPU с применением llama.cpp	12
- Установка и настройка Langchain	13
...
Для ознакомления с возможностями LLM Qwen - Qwen3-8B — идеальный стартовый вариант: 8 млрд параметров, поддержка 119 языков, контекст до 128K токенов, обучена на ~36 трлн токенов (qwenlm.github.io). Хорошо справляется с общими задачами, кодом и рассуждениями.
...
RTX3060 для домашних компьютеров, не оптимизирована для работы 24/7. 
A4000 для профессиональных машин
...
При запуске на CPU модель полностью загружается в оперативную память (а не в видеопамять). Если памяти окажется недостаточно, то система начнет использовать своп-память и скорость упадет на порядки.
...
Наилучшим вариантом является отдельный сервер с GPU, однако такой вариант как правило недоступен, кроме того, в лабораторных работах понадобится несколько серверов. 
Поэтому наилучшим вариантом является виртуализация на рабочем компьютере, либо аренда виртуальных мощностей.
...
Установка драйверов NVIDIA для различных моделей GPU (Tesla, Turing, Ampere, Volta и др.) для разных операционных систем (Ubuntu 24.04 LTS и др.) требует внимания к архитектуре карты и особенностям операционной системы. Рекомендуется перед установкой изучить рекомендации на сайте nvidia и на сайтах разработчиков используемой операционной системы. Например
...
ollama
Подходит для первичного знакомства, втоматически скачивает квантованную модель (GGUF), минимум конфигурации, работает даже на CPU. Подходит для быстрого тестирования

https://ollama.com/download
для убунту надо выбрать вариант с командой curl
проанализировать содержание файла install.sh
...
Проанализировать: Сколько заняла места? В какие папки произошла инсталляция?
...
AWQ-квантование сохраняет ~95% качества при 4-bit 
Модели доступны на Hugging Face Hub: Qwen/Qwen3-8B-AWQ, Qwen/Qwen3-14B-AWQ 
...
Для тестирования без GPU попробуйте llama.cpp + GGUF — работает даже на мощном ноутбуке
